  ...
  s.str_items |> concat_map @@ fun x -> match x.str_desc with
    ...
    | Tstr_value (_, list) ->
        concat_map (fun (p,e) ->
          ...
          in
          extract (p, Some e)) list
    | Tstr_primitive (id, {loc}, vdesc) -> 
        [ A_str_primitive (id, loc, vdesc.val_val.val_type) ]
    | Tstr_type xs ->
        xs |> map @@ fun (id, {loc}, td) ->
          A_str_type (id, loc, 
                      { a_typ_params = map (Option.map (fun {txt} -> txt)) td.typ_params;
                        a_typ_kind = assert false;
                        a_typ_private = td.typ_private;
                        a_typ_manifest = Option.map (fun x -> x.ctyp_type) td.typ_manifest })

    | Tstr_exception (id, {loc}, {exn_params}) -> 
        [ A_str_exception (id, loc, map (fun x -> x.ctyp_type) exn_params) ]
    | Tstr_exn_rebind (id, {loc}, p, longid) ->
