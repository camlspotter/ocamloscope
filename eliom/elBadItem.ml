(* This is only for demo purpose. What would look like if there is no item grouping *)
...
let entry_group_body_elem (_id, (i : Item.t)) =
...
let to_bad : is -> bad_is = fun (_, _, xs) -> 
  List.concat_map (fun (_pkgs, iis) -> iis) xs

(*
let entry_group dist (is : bad_is) =
...
let group (ii : int * Item.t) =
  tr [ td_class "entry" [ entry_group_body_elem ii ]
     ]
